...
A PRD feels finished the moment the Notion page is shared. Collect comments, update the document, hand it to engineering, then carry the rest in your head: who still needs to know that a requirement moved, which version the team actually agreed to. That is how most product organisations run, and it is how we ran at Zerion.

It held up fine while LLMs lived in the browser. I pasted context into a chat, copied the output back into Notion, and did it again next session. Then models like Claude Opus made agent development practical, and the mismatch stopped being tolerable. An agent could reason across an entire codebase. The reasoning behind the product was still sitting in pages someone had to find and paste by hand.
...
Then I checked it at the aggregate level, and the failure rate looked like noise.
...
So I decomposed it instead: the funnel by stage, then the failures by cause, by network, and by the kind of asset being traded. That last cut is where it appeared. Small cap assets were failing at roughly 15%, against an average that looked like nothing. The reason was not subtle once it was visible: they are far more volatile than the majors, and we applied one largely static slippage tolerance to everything.
...
Four words, and the same commit added the two numbers that made them enforceable: the price users actually got against the price they were quoted, and the revenue each trade produced. One metric had to fall while those two held. Without them the target is trivially gameable, because you can drive failures to zero by widening the tolerance until every trade succeeds and every user overpays.
...
That is the real limit of building on the developers' substrate: it serves the people already standing on it. Extending the same decision trail out to marketing — one context, one source of the promise, no manual re-entry — is the piece I would build first if I were starting this again.

## the contract held me to it too
...
The Auto-slippage contract makes the same point at a smaller scale. Its primary metric closed convincingly. The execution-quality guardrails the review had insisted on were never fully closed out, so by the contract's own decision rule the honest conclusion is "keep going and finish the evidence," not "ship it and scale." I would rather have a contract that can hold its author to that than one that lets me round up. A rule that cannot inconvenience the person who wrote it is decoration.
